...
import javatuples.valueintf.IValueKey;
import javatuples.valueintf.IValueValue;

/**
 * <p>
 * A tuple of two network.elements, with positions 0 and 1 renamed as "key" and
 * "value", respectively.
 * </p>
 * 
 * @since 1.0
...
public final class KeyValue<A, B> extends Tuple implements IValueKey<A>, IValueValue<B> {
	...
	public static <X> KeyValue<X, X> fromArray(final X[] array) {
		...
		if (array.length != 2) {
			throw new IllegalArgumentException(
					"Array must have exactly 2 network.elements in order to create a KeyValue. Size is "
							+ array.length);
		}
	...
	private static <X> KeyValue<X, X> fromIterable(final Iterable<X> iterable, int index, final boolean exactSize) {
		...
		if (tooFewElements && exactSize) {
			throw new IllegalArgumentException("Not enough network.elements for creating a KeyValue (2 needed)");
		}

		if (iter.hasNext() && exactSize) {
			throw new IllegalArgumentException(
					"Iterable must have exactly 2 available network.elements in order to create a KeyValue.");
		}
